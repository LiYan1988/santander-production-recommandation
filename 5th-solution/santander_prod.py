...
#################################### Prepare lag features and targets #######################

# Use shift to construct lag features for target_cols
# First find out which rows have conflictions in shifting, these samples are rows
# that generate invalid results for shifting. Because one customer's data is
# shifted to another customer's row.
...
# Use shift to construct lag features of target_cols
for col in target_cols:
    for shift_val in range(1,18):
...
# Calculate added products for the current month
for col in target_cols:
...
# The first month that each customer appear
